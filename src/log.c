...
{
   FixedFont* ff = &c->font.fixed_font;

   assert (ff->glyph_count > 0);
   assert (PIXEL_BUFFER_WIDTH - PADDING_LR >= (size_t)ff->glyph_width);
   assert (PIXEL_BUFFER_HEIGHT - PADDING_TB >= (size_t)ff->glyph_height);

   s_page_col_count = (PIXEL_BUFFER_WIDTH - PADDING_LR - ff->glyph_width) /
                      (ff->glyph_width + ff->glyph_spacing);
   s_page_col_count += 2;

   s_page_row_count = (PIXEL_BUFFER_HEIGHT - PADDING_TB - ff->glyph_height) /
                      (ff->glyph_height + ff->glyph_spacing);
   s_page_row_count += 1;
...
{
   FixedFont* font =
      is_inverted ? &c->font.inverted_fixed_font : &c->font.fixed_font;

   for (size_t i = 0; i < s_page_row_count; ++i)
   {
      const char* current_line =
         &s_log_buffer[(c->state.current_log_page - 1) * s_page_size +
                       i * s_page_col_count];
      const int origin_y =
         PADDING_TOP + (int)i * (font->glyph_height + font->glyph_spacing);
...
draw_osd (Context* c, Image* buf, bool is_inverted)
{
   ...
   for (size_t i = 0; i < s_page_row_count; ++i)
   {
      const char* current_line = &s_osd_buffer[i * s_page_col_count];
      const int   origin_y =
         PADDING_TOP + (int)i * (font->glyph_height + font->glyph_spacing);
